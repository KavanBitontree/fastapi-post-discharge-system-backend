...
def calculate_chunking_strategy(
    ...
    max_chunks_per_batch = max(1, min(
        config.rpm,
        int(config.tpm / tokens_per_request),
    ))
...
def _get_recommendation(strategy: ChunkingStrategy) -> str:
    ...
    if strategy.estimated_total_chunks == 1:
        ...
        return (
            f"Large document: {strategy.estimated_total_chunks} chunks. "
            "Processing may take several minutes. Consider batch processing."
        )
